...
from models.utils_simgcd import info_expanded_negative_logits
from models.utils_simgcd_pro import get_kmeans_centroid_for_new_head
from models.utils_proto_aug import ProtoAugManager
from models.vc import VCClassifier, WeightGenerator, VCHAP  # Import VC modules
from models import vision_transformer as vits
from config import dino_pretrain_path, exp_root_vc_cgcd

import warnings
warnings.filterwarnings("ignore", category=DeprecationWarning)
warnings.filterwarnings("ignore", category=FutureWarning)

def build_weak_transform(image_size, args):
    """
    Teacher model - weak augmentation
    """
    return transforms.Compose([
        transforms.Resize(int(image_size)),   # Equivalent batch_pad fixed_size
...
def build_strong_transform(image_size, args):
    """
    Student model - strong augmentation
    """
    return transforms.Compose([
        transforms.Resize(int(image_size / args.crop_pct), interpolation=args.interpolation),
        transforms.RandomCrop(image_size, padding=4),
        transforms.RandomHorizontalFlip(p=0.5),
        transforms.ColorJitter(brightness=0.8, contrast=0.5, saturation=0.5, hue=0.25),
        transforms.RandomGrayscale(p=0.2),  
        transforms.GaussianBlur(kernel_size=3, sigma=(0.1, 2.0)),
        transforms.RandomAffine(degrees=(-15, 15), translate=(0.1, 0.1), scale=(0.8, 1.2)),
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.485, 0.456, 0.406],
                             std=[0.229, 0.224, 0.225]),
        transforms.RandomErasing(scale=(0.02, 0.4), ratio=(0.3, 3.3), p=0.5)
    ])

def tensor_to_pil(img_tensor):
    """Convert (B, C, H, W) Tensor to list[PIL]"""
    img_list = []
    for img in img_tensor:               # img: (C, H, W)
        img_list.append(TF.to_pil_image(img.cpu()))
    return img_list

def pil_list_to_tensor(pil_list, transform):
    """Convert list[PIL] to (B, C, H, W) Tensor"""
    tensor_list = [transform(pil) for pil in pil_list]
    return torch.stack(tensor_list).cuda(non_blocking=True)

def VCloss(
    y_1: torch.Tensor,          # [N], teacher top1 predicted labels (used as pseudo labels)
    logits: torch.Tensor,       # [N, K+1], last column is virtual class
    targets: torch.Tensor,      # [N], ground truth labels (only used for labeled data); use ignore_index for unlabeled
    pc_set: torch.Tensor,       # [N, M], potential class indices for each sample (-1 for padding)
    is_confusing: torch.Tensor, # [N] bool, True if the sample is confusing
    ignore_index: int = 255,
    reduction: str = "mean"
) -> torch.Tensor:
    """
    VC-style loss implementation (based on paper and source code)
    Behavior:
      - For confusing samples:
          * Ignore logits in potential-category set (don't participate in softmax)
          * Set target label to virtual class (index = K)
      - For non-confusing samples:
          * If targets provide valid labels (0..K-1 and != ignore_index), use it as target
          * Otherwise use teacher prediction y_1 as pseudo label
    """
    B, K_plus_1 = logits.shape
    device = logits.device
    loss = torch.tensor(0.0, device=device)

    if y_1 is None:
        raise ValueError("y_1 (teacher top1) is required")
    if targets is None:
        targets = torch.full((B,), -1, dtype=torch.long, device=device)
    else:
        targets = targets.to(device)

    # Prepare pc_set mask: allowed_mask True means this logit column participates in softmax/CE
    allowed_mask = torch.ones((B, K_plus_1), dtype=torch.bool, device=device)
    K = K_plus_1 - 1

    if pc_set is not None:
        pc = pc_set.to(device).long()
        for i in range(B):
            if bool(is_confusing[i].item()):
                row = pc[i]
                valid = row[(row >= 0) & (row < K)]
                if valid.numel() > 0:
                    allowed_mask[i, valid.tolist()] = False
    ...
    # Construct target indices
    target_ext = torch.full((B,), K, dtype=torch.long, device=device)
    y1 = y_1.to(device).long()
    for i in range(B):
        if bool(is_confusing[i].item()):
            target_ext[i] = K
        else:
            t = targets[i].long()
            if (t >= 0) and (t < K) and (t != ignore_index):
                target_ext[i] = t
            else:
                target_ext[i] = y1[i]

    # Mask logits by setting disallowed positions to very negative value
    logits_masked = logits.clone()
    logits_masked[~allowed_mask] = -1e9

    loss = F.cross_entropy(logits_masked, target_ext, reduction=reduction)
...
def train_offline(student, train_loader, test_loader, args):
    ...
    exp_lr_scheduler2= lr_scheduler.CosineAnnealingWarmRestarts(optimizer, 
                                        T_0 = 30,# Number of iterations for the first restart
                                        T_mult = 1, # A factor increases TiTi​ after a restart
                                        eta_min = args.lr * 1e-3) # Minimum learning rate
...
def train_online(student, student_pre, proto_aug_manager, train_loader, test_loader, current_session, args):
    ...
    feature_bank_dim = args.feat_bank_size
    no_bs = int(feature_bank_dim // args.batch_size)
    feat_bank = torch.rand(no_bs, args.batch_size, 256).to(device)
    ...
    # Initialize student and teacher models with DINOHead as classifier
    student_backbone = student[0]
    student_classifier = DINOHead(in_dim=768, out_dim=args.mlp_out_dim_cur)
    student_model = nn.Sequential(student_backbone, student_classifier)

    teacher_backbone = deepcopy(student_backbone)
    teacher_classifier = DINOHead(in_dim=768, out_dim=args.mlp_out_dim_cur)
    #print(f'Initializing teacher model with {args.mlp_out_dim_cur} output dimensions.')
    teacher_model = nn.Sequential(teacher_backbone, teacher_classifier)
    
    # student_model = deepcopy(student)
    # teacher_model = deepcopy(student)

    # Initialize teacher model weights from student model weights
    for param_q, param_k in zip(student_model.parameters(), teacher_model.parameters()):
        param_k.data.copy_(param_q.data)
        param_k.requires_grad = False

    weak_transform = build_weak_transform(args.image_size, args)
    strong_transform = build_strong_transform(args.image_size, args)
    
    vchap = VCHAP(student_model, teacher_model, 768)
    vchap.to(device)
    
    # # Initialize Excel logging
    # excel_dir = os.path.join(args.exp_root, args.dataset_name, args.exp_name)
    # os.makedirs(excel_dir, exist_ok=True)
    # excel_path = os.path.join(excel_dir, f'confusing_ratio_log_session_{current_session}.xlsx')
    # confusing_ratio_records = []  # Store all records
    ...
    for epoch in range(args.epochs_online_per_session):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            # Convert to PIL list
            pil_list = tensor_to_pil(images)
            # Weak augmentation -> teacher (B, C, H, W)
            images_weak = pil_list_to_tensor(pil_list, weak_transform)
            # Strong augmentation -> student (B, C, H, W) (256,3,224,224)
            images_strong = pil_list_to_tensor(pil_list, strong_transform)

            # Compute virtual weights
            student_weights = student_model[1].last_layer.weight # [K_total, feat_dim 768]
            student_features = student_model[0](images_strong)
            student_features = F.normalize(student_features, dim=-1) # [B, feat_dim] (256,768)
            student_logits = student_model[1](student_features)  # the shape of student_logits[1] is [B, K]
            with torch.no_grad():
                teacher_features = teacher_model[0](images_weak)
                teacher_features = torch.nn.functional.normalize(teacher_features, dim=-1)
                
            # Use VCClassifier to handle confusing samples
            class_weights = student_model[1].last_layer.weight.data
            y_1,is_confusing, pc_set, virtual_weights, confusing_ratio  = vchap.forward(images_strong, images_weak, class_weights)
            
            # Compute virtual class logits: cosine similarity
            virtual_logits = torch.einsum("bf,bf->b", student_features, virtual_weights)  #[B 256]
            logits_with_vc = torch.cat([student_logits[1], virtual_logits.unsqueeze(-1)], dim=1)  # [B, K+1]
            vc_loss = VCloss(y_1, logits_with_vc, class_labels.repeat(2), pc_set, is_confusing,ignore_index=-1,reduction="mean")
            
            # Modified vchap.forward call to receive confusing_ratio
            y_1, is_confusing, pc_set, virtual_weights, confusing_ratio = vchap.forward(
                images_strong, images_weak, class_weights
            )
            
            # # Record confusing_ratio for current batch
            # confusing_ratio_records.append({
            #     'session': current_session,
            #     'epoch': epoch,
            #     'batch_idx': batch_idx,
            #     'confusing_ratio': confusing_ratio,
            #     'batch_size': len(class_labels)
            # })            
            
            student_proj, student_out = student(images)
            #print(f'student_out shape: {student_out.shape}')
            teacher_out = student_out.detach() 
            ...
            #print(f'avg_probs_old_in shape: {avg_probs_old_in.shape} | avg_probs_new_in shape: {avg_probs_new_in.shape}')
            ...
            # contrastive_logits, contrastive_labels = info_nce_logits(features=student_proj)
            # contrastive_loss_orig = torch.nn.CrossEntropyLoss()(contrastive_logits, contrastive_labels)
            # AMEND Extended neighborhood contrastive loss
                        
            indx = int(batch_idx % no_bs)
            with torch.no_grad():
                student_proj_feat = student_proj.detach().chunk(2)
                student_proj_feat=student_proj_feat[0]
                student_proj_norm= torch.nn.functional.normalize(student_proj_feat, dim=1)
                feat_bank[indx]=student_proj_norm.clone()
                
            feat_bank_reshape = feat_bank.reshape(feature_bank_dim, 256)
            contrastive_logits, contrastive_label, contrastive_label1, expand_no_neighbors = info_expanded_negative_logits(
                features=student_proj,
                feat_bank=feat_bank_reshape,
                K=args.neighbors,
                expanded_K=args.expanded_neighbor
            )
            contrastive_label = contrastive_label.to(device)
            contrastive_label1 = contrastive_label1.to(device)
            contrastive_loss = torch.nn.CrossEntropyLoss()(contrastive_logits, contrastive_label)
            for i in range(args.neighbors + expand_no_neighbors):
                temp = (i + 1) * contrastive_label1
                if i < args.neighbors:
                    contrastive_loss += torch.nn.CrossEntropyLoss()(contrastive_logits, temp)
                else:
                    contrastive_loss += 0.1 * torch.nn.CrossEntropyLoss()(contrastive_logits, temp)
            contrastive_loss = contrastive_loss / (1 + args.neighbors + 0.1 * expand_no_neighbors)
            ...
            loss = 0
            loss += 1 * cluster_loss
            loss += 0.8 * contrastive_loss
            loss += args.proto_aug_weight * proto_aug_loss
            loss += args.feat_distill_weight * feat_distill_loss
            loss += args.vc_weight * vc_loss #default args.vc_weight=0.15

            # Update student model of VCHAP
            with torch.no_grad():
                for param_q, param_k in zip(student_model.parameters(), teacher_model.parameters()):
                    param_k.data = param_k.data * args.ema_momentum + param_q.data * (1. - args.ema_momentum)
            ...
            pstr += f'vc_loss: {vc_loss.item():.4f} '
        ...
        all_acc_test, old_acc_test, new_acc_test, \
            all_acc_soft_test, seen_acc_test, unseen_acc_test, new_as_old_ratio = test_online(student, test_loader, epoch=epoch, save_name='Test ACC', args=args)
        args.logger.info('Test Accuracies (Hard): All {:.4f} | Old {:.4f} | New {:.4f} | New→Old Ratio: {:.2f}%'.format(all_acc_test, old_acc_test, new_acc_test, new_as_old_ratio))
        args.logger.info('Test Accuracies (Soft): All {:.4f} | Seen {:.4f} | Unseen {:.4f}'.format(all_acc_soft_test, seen_acc_test, unseen_acc_test))

        new_as_old_records.append({
            'session': current_session,
            'epoch': epoch,
            'new_as_old_ratio': new_as_old_ratio
        })
        ...
        # if len(confusing_ratio_records) > 0:
        #     df = pd.DataFrame(confusing_ratio_records)
        #     df.to_excel(excel_path, index=False)
        #     args.logger.info(f'Saved confusing_ratio log to {excel_path}')
            
        # if len(new_as_old_records) > 0:
        #     excel_dir1 = os.path.join(args.exp_root, 'cifar100', 'New→Old Ratio')  # User specified path
        #     os.makedirs(excel_dir1, exist_ok=True)
        #     excel_path1 = os.path.join(excel_dir1, 'new_as_old_ratio_summary.xlsx')
        #     df1 = pd.DataFrame(new_as_old_records)
        #     df1.to_excel(excel_path1, index=False)
        #     args.logger.info(f'Saved New→Old Ratio summary to {excel_path1}')
...
def test_online(model, test_loader, epoch, save_name, args):
    ...
    new_samples_mask = ~mask_hard.astype(bool)
    if new_samples_mask.sum() > 0:
        # Ground truth is new class but predicted as old class (predicted label within initial old class range)
        new_as_old_mask = new_samples_mask & (preds < len(args.train_classes))
        new_as_old_ratio = new_as_old_mask.sum() / new_samples_mask.sum() * 100
    else:
        new_as_old_ratio = 0.0

    return all_acc, old_acc, new_acc, all_acc_soft, seen_acc, unseen_acc, new_as_old_ratio
'''====================================================================================================================='''
...
if __name__ == "__main__":
    ...
    parser.add_argument('--exp_root', type=str, default=exp_root_vc_cgcd)
    ...
    parser.add_argument('--ema_momentum', type=float, default=0.9) #upgreade ts framework
    parser.add_argument('--vc_weight', type=float, default=0.1)
    
    #AMEND
    parser.add_argument('--lam',type=int)    
    # parser.add_argument('--aug_epochs', default=100, type=int)
    # parser.add_argument('--nearest_epochs', default=100, type=int)    
    parser.add_argument('--neighbors', default=4, type=int)
    parser.add_argument('--feat_bank_size', default=2048, type=int)
    parser.add_argument('--expanded_neighbor',default=1,type=int)
    ...
    args.exp_name = 'vchap' + '-' + args.train_session
    ...
    init_experiment(args, runner_name=['VCHap'])
        ...
        for session in range(start_session, args.continual_session_num):
            ...
            if session == 0:
                ...
                if args.load_offline_id is not None:
                    load_dir_online = os.path.join(exp_root_vc_cgcd + '_' + 'offline', args.dataset_name, args.load_offline_id, 'checkpoints', 'model_best.pt')
            ...
            if args.init_new_head:
                new_head, results = get_kmeans_centroid_for_new_head(model_pre, online_session_train_loader_for_new_head_init, args, device)   # torch.Size([10, 768])
